  ...
  useEffect(() => {
    ...
    if (video.canPlayType("application/vnd.apple.mpegurl")) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        console.log("HLS Manifest Parsed - Discovering tracks...");
      ...
      {(audioTracks.length > 1 || subTracks.length > 0) && (
        <div className="absolute top-4 right-4 z-50">
          <button
            ...
            className="w-10 h-10 bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-xl flex items-center justify-center text-white transition-all border border-white/10 active:scale-95 cursor-pointer shadow-lg"
          ...
          {showSettings && (
            <div className="absolute top-12 right-0 w-56 bg-black/80 backdrop-blur-2xl rounded-2xl border border-white/10 p-4 shadow-2xl animate-in fade-in slide-in-from-top-2 duration-300 overflow-hidden">
              {/* Audio Tracks */}
